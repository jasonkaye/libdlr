...
def solve_syk_fwd_iter(d, mu, beta=1., U=0.1, g0_l=None, max_iter=1000, tol=1e-14, mixing=0.5, verbose=False):
...
if __name__ == '__main__':
    ...
    g_l_fwd = solve_syk_fwd_iter(d, mu0, beta=beta, U=U, tol=tol, mixing=mixing, max_iter=max_iter, verbose=True)
    g_l_root_dyson = solve_syk_root(d, mu0, beta=beta, U=U, tol=tol * 1e-2, verbose=True)
    
    mu_vec = np.linspace(-0.01, 0.01, num=10)
    density_vec = np.zeros_like(mu_vec)

    for idx, mu in enumerate(mu_vec):
        g_l = solve_syk_fwd_iter(d, mu, beta=beta, U=U, tol=tol, mixing=mixing, max_iter=max_iter) 
